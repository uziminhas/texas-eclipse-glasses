...
function DefaultCheckoutButton(
  ...
  const linkHref =
    props.plan.name === 'single'
      ? `${configuration.paths.paymentLinkSingle}`
      : props.plan.name === 'fivePack'
      ? `${configuration.paths.paymentLinkFivePack}`
      : props.plan.href
      ? props.plan.href
      : ``;

  const label = props.plan.label ?? 'Buy now';
